...
\title{Predictive Closure and the Conversion Scale: When Zeta-Mass Matching Becomes Testable}
...
\section{Core calibration equation}
From Paper 4,
\[
...
\section{Descriptive versus predictive use of $L_*$}
\begin{definition}[Descriptive scaling]
A fit is descriptive if $L_*$ is chosen independently for each mass target.
\end{definition}

\begin{definition}[Predictive scaling]
A fit is predictive if $L_*$ is fixed independently of the target masses before comparison.
\end{definition}

\begin{definition}[Family-stabilized scaling]
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.83\textwidth]{../assets/figures/calibration-regimes.svg}
\caption{Predictive strength increases as freedom in $L_*$ decreases.}
\end{figure}

\section{Testing protocols}
\paragraph{Protocol A: Fixed-scale prediction.}
...
\paragraph{Protocol B: Family-stabilized scale.}
...
\paragraph{Protocol C: Out-of-sample mass matching.}
...
\paragraph{Protocol D: Null tests.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\textwidth]{../assets/figures/mass-to-zeta-inverse-test.svg}
\caption{Inverse evaluation pipeline for null and out-of-sample testing.}
\end{figure}
