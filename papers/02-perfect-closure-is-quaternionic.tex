...
\title{Quaternionic Closure Slices: The Critical Line as a Complex Trace of $\Re(q)=1/2$ Geometry}
...
\noindent\textbf{Keywords:} quaternions; critical line; slice geometry; perfect closure; six-step cadence; completed residual.
...
Our goal is not to replace complex analysis. Our goal is to show that the classical critical line can be read as one trace of a broader hyperslice $\Re(q)=1/2$.

\section{Contributions}
\begin{enumerate}
  \item We define the quaternionic closure hyperslice $\Sigma_{1/2}=\{q\in\mathbb H:\Re(q)=1/2\}$ and its affine traces $q_{\mathbf u}(t)=1/2+\mathbf u t$.
  \item We separate affine slice coordinates from unit-phase dynamics.
  \item We formalize the six-step closure cadence as geometric intuition, not a zeta-zero theorem.
  \item We show the norm identity $\|\xi_{\mathbb H}(1/2+\mathbf u t)\|^2=|\Xi(t)|^2$.
\end{enumerate}

\section{Preliminaries and Mathematical Setup}
...
\caption{The classical critical line appears as the $\mathbf u=i$ trace of the quaternionic family $q_{\mathbf u}(t)$.}
\end{figure}

\section{Formal Definitions and Propositions}
Define:
\begin{itemize}
  \item affine slice coordinate: $q=1/2+\mathbf u t$;
  \item unit phase: $Q=e^{\mathbf u\phi}=\cos\phi+\mathbf u\sin\phi$.
\end{itemize}
...
\begin{proof}
The cadence is a group-geometric statement on unit quaternions. Zero conditions for $\Xi$ are analytic statements for the completed zeta residual. They are compatible but distinct levels.
\end{proof}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{../assets/figures/six-step-closure-gate.svg}
\caption{Six-step cadence for $Q=e^{\mathbf u\pi/3}$, illustrating $Q^3=-1$ and $Q^6=1$.}
\end{figure}

\section{Interpretation and Discussion}
Lift the prime-phase factor:
\[
p^{-(1/2+it)}=p^{-1/2}e^{-it\log p}
\quad\leadsto\quad
p^{-(1/2+\mathbf u t)}=p^{-1/2}e^{-\mathbf u t\log p}.
\]
Amplitude remains $p^{-1/2}$; only orientation axis changes.

Let $\Xi(t)=\xi(1/2+it)$, and define
\[
...
\]
So scalar closure score is slice-invariant under this embedding.

\section{Scope and Limitations}
\begin{itemize}
  \item We do \textbf{not} claim quaternionic geometry replaces complex analysis.
  \item We do \textbf{not} claim the six-step cadence proves zeta-zero placement.
  \item We do \textbf{not} claim a full quaternionic analytic continuation theory for $\zeta$.
  \item We do \textbf{not} claim mass predictions in this paper.
\end{itemize}

\section{Conclusion}
We defined quaternionic closure slices and showed that the classical critical line is one trace through $\Re(q)=1/2$ geometry. We distinguished affine slicing from unit-phase dynamics and formalized six-step cadence as geometric intuition. We then showed norm preservation for the lifted residual, providing the compatibility structure needed for Paper 3 and the operator bridge in Paper 4.
